...
export class RestaurantDetailsComponent implements OnInit {
  ...
  getRestaurant(): void {
    ...
    const id = this.route.snapshot.paramMap.get('id')
    this.service.getRestaurant(id).subscribe((restaurant) => {
      this.restaurant = restaurant
      console.log('restaurant: ' + JSON.stringify(this.restaurant))
      this.getReviewforCurrentUser()
    })
  }
  ...
  getReviewforCurrentUser(): void {
    ...
    this.reviewService
      .getReviewforRestaurantandUser(restaurant, user)
      .subscribe((currentUserReview) => {
        this.currentUserReview = currentUserReview[0]
        this.getReviewsforRestaurant()
      })
  }
