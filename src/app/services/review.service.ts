...
export class ReviewService {
  ...
  getReview(id: string): Observable<Review> {
    this.messageService.add(`Review Service: fetched review id=${id}`);
  ...
  getReviewsforRestaurant(restaurant: Restaurant): Observable<Review[]> {
    ...
    return this.http.get<Review[]>(url).pipe(
      catchError(this.handleError<Review[]>(`getReviewsforRestaurant`))
    );
  }

  getReviewforRestaurantandUser(restaurant: Restaurant, user: User): Observable<Review> {
